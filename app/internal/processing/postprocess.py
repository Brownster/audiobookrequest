...
class PostProcessor:
    ...
    async def _merge_with_ffmpeg(self, files: List[Path], destination: Path) -> None:
        ...
        if missing:
            raise PostProcessingError(f"Audio files not found: {', '.join(str(f) for f in missing[:3])}")
        ...
        logger.info(
            "PostProcessor: merging audio with ffmpeg",
        ...
        if not destination.exists():
            raise PostProcessingError(f"ffmpeg completed but output file not created: {destination}")

        file_size = destination.stat().st_size
        if file_size == 0:
            raise PostProcessingError(
                f"ffmpeg created empty file. stderr: {stderr.decode()[:500]}"
            )

        logger.info(
            "PostProcessor: merge complete",
            output=str(destination),
